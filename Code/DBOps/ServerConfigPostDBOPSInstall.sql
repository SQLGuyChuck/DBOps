...
EXEC msdb.dbo.sp_add_operator @name=N'DBOps', 
		...
		@pager_days=0, 
		@email_address=N'alerts@yourdomain.xyz', 
...
--copied from: \DBAScripts\ServerConfigChecksandFixes\Should we use optimize for ad hoc workloads.sql

/*
--http://msdn.microsoft.com/en-us/library/ee343986%28SQL.100%29.aspx
...
--This is available for SQL 2008+

--Cheap and easy way to find when SQL was last restarted. Hopefully cache has not been flushed since then.
IF EXISTS (Select create_date, DateDiff(dd,create_date,Getdate()) DaysServerRunning
from sys.databases 
where name = 'tempdb'
and DateDiff(dd,create_date,Getdate()) < 7)
Select 'Do not trust this output until SQL Server has been running for at least 1 week.'

-- Do not run this TSQL until SQL Server has been running for at least 1 week
SET NOCOUNT ON
SELECT objtype AS [Cache Store Type],
        ...
        AVG(cast(usecounts as bigint)) AS [All Plans - Ave Use Count],
...
;WITH CTE (AdhocSize, TotalSize)
as (
SELECT SUM(CAST((CASE WHEN usecounts = 1 AND LOWER(objtype) = 'adhoc' THEN size_in_bytes ELSE 0 END) as decimal(14,2))) / 1048576 ,
        SUM (CAST (size_in_bytes as decimal (14,2))) / 1048576 as [Total cache plan size (MB)]
FROM sys.dm_exec_cached_plans )
Select AdhocSize as [Current memory occupied by adhoc plans only used once (MB)], TotalSize as [Total cache plan size (MB)],
AdhocSize/TotalSize * 100.0 as [% of total cache plan occupied by adhoc plans only used once], 
CASE WHEN (AdhocSize / TotalSize )* 100 < 25 THEN 'Things are fine with adhoc size being less than 25%.' 
WHEN (AdhocSize / TotalSize )* 100 BETWEEN 25 and 35 THEN 'Seriously consider changing to using ''Optimize for ad hoc workloads'' with such a high % of single use plans in your cache.' 
ELSE 'Definately enable server setting ''Optimize for ad hoc workloads'' as it will make a significant difference:
EXEC sys.sp_configure N''optimize for ad hoc workloads'', N''1''
RECONFIGURE WITH OVERRIDE' END as [Recommendation]
FROM CTE
--To see all the sp_configure values:
exec sp_configure 'show advanced options' , 1
reconfigure
