...
EXEC msdb.dbo.sp_delete_job @job_name=N'DBA: Monitor Blocking', @delete_unused_schedule=1
GO
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA: Monitor Blocking', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Monitor for blocking and log to dbops.dbo.DBA_BlockingInfo table.', 
		@category_name=N'Database Maintenance', 
		@owner_login_name=N'sa',
		@notify_email_operator_name=N'ITDBOpsAlerts', 
		@job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
select @Blockingms=dbops.dbo.udf_GetProcessParameter (''Admin'',''Blocking Threshold (ms)''),
	 @EscalationEmail=dbops.dbo.udf_GetProcessParameter (''Admin'',''DBOPS Team Escalation'')
...
Begin
	...
	select @maxident = max(ident) from dbo.DBA_BlockingInfo
	...
	select @tableHTML = ''Threshold is '' + @Blockingms + '' ms. To see all that was running during blocking capture, please run this query:</p>''  + 
					+ ''<p>SELECT * FROM DBOPS.dbo.DBA_BlockingInfo <br />'' + 
...
EXEC msdb.dbo.sp_update_job @job_name=N'DBA: Monitor Blocking', 
		@notify_level_email=2, 
