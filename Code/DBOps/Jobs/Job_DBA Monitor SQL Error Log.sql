...
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DBA: Monitor SQL Error Log')
BEGIN
	Set @jobexists = 1
	GOTO EndSave
END
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA: Monitor SQL Error Log', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Run prc_CheckSQLErrorLog which looks in the SQL error log for non-excluded entries and send an email with result, if any.', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run proc', 
		...
		@command=N'exec dbops.dbo.prc_Check_SQLErrorLog', 
		@database_name=N'DBOps', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Monitor SQL Error Log', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
EndSave:
If @jobexists = 1
	Print 'Job DBA: Monitor SQL Error Log exists, so skipping' 
GO
